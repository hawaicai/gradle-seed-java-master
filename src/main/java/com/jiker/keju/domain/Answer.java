...
public class Answer {
    ...
    private boolean isSamePosition(final String inputAnswer, final String number) {
        return getAnswerNumbers(this.value).indexOf(number) == getAnswerNumbers(inputAnswer).indexOf(number);
    }

    private List<String> getAnswerNumbers(String value) {
        return newArrayList(value.split(AnswerConstant.ANSWER_SEPARATOR));
    }
